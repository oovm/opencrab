...
async fn main() -> Result<()> {
    ...
    let _config_loader = ConfigLoader::new();
    ...
    let _skill_service = MemorySkillService::new();
    println!("  ✓ MemorySkillService 初始化成功");

    let _memory_service = MemoryMemoryService::new();
    println!("  ✓ MemoryMemoryService 初始化成功");

    let _chat_service = MemoryChatService::new();
    println!("  ✓ MemoryChatService 初始化成功");

    let _tool_service = MemoryToolService::new();
    println!("  ✓ MemoryToolService 初始化成功");

    let _scheduler_service = MemorySchedulerService::new();
    println!("  ✓ MemorySchedulerService 初始化成功");

    let _workspace_service = MemoryWorkspaceService::new();
    ...
    let agent = agent_service
        .create_agent("Test Agent", Some("一个测试用的智能体"), AgentType::General)
        .await?;
